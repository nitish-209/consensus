...
class Consensus(Node):
    ...
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.cmd_vel_pub4 = self.create_publisher(Twist, 'tb3_4/cmd_vel', qos)
    ...
    *******************************************************************************"""
    ...
    def odom_callback4(self, msg):
        self.scan_odom4 = msg
        self.x4 = self.scan_odom4.pose.pose.position.x
        self.init_odom_state3 = True

    def consensus_callback(self):
        t1,t2,t3,t4 = Twist(),Twist(),Twist(),Twist()

        t1.linear.x = (self.x2 + self.x3 - 2*self.x1)
        t2.linear.x = (self.x1 + self.x3 - 2*self.x2)
        t3.linear.x = (self.x1 + self.x2 + self.x4 - 3*self.x3)
        t4.linear.x = (self.x3 - self.x4)
       
        if abs(t1.linear.x) > 0.22:
        ...
        if abs(t3.linear.x) > 0.22:
            ...
            t3.linear.x = 0.0
        if abs(t4.linear.x) > 0.22:
            t4.linear.x = sign(t4.linear.x) * 0.20
        elif abs(t4.linear.x) <0.01:
            t4.linear.x = 0.0

        print(self.x1,self.x2,self.x3,self.x4)
        print(t1.linear.x,t2.linear.x,t3.linear.x,t4.linear.x)

        self.cmd_vel_pub1.publish(t1)
        self.cmd_vel_pub2.publish(t2)
        self.cmd_vel_pub3.publish(t3)
        self.cmd_vel_pub4.publish(t4)
